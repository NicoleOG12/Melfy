...
function exibirInformacoes() {
  ...
  document.getElementById("sobrenome-input").value =
    usuarioAtual.nome.trim().split(" ").slice(1).join(" ") || "";
  document.getElementById("email-input").value = usuarioAtual.email || "";
  document.getElementById("celular-input").value = formatarCelular(
    usuarioAtual.telefone || "",
  );
  document.getElementById("cpf-input").value = formatarCPF(
    usuarioAtual.cpf || "",
  );
  document.getElementById("dataNascimento-input").value =
    dataISO(usuarioAtual.data_nasc) || "";
}
...
function salvarDados() {
  ...
  const novaListaUsuarios = listaUsuarios.map((u) =>
...
window.mostrarSecao = function (secao) {
  document.getElementById("secao-dados").style.display =
    secao === "dados" ? "block" : "none";
  document.getElementById("secao-enderecos").style.display =
    secao === "enderecos" ? "block" : "none";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (celularInput)
    celularInput.addEventListener(
      "input",
      (e) => (e.target.value = formatarCelular(e.target.value)),
    );
  ...
  if (cpfInput)
    cpfInput.addEventListener(
      "input",
      (e) => (e.target.value = formatarCPF(e.target.value)),
    );

  const numCartao = document.getElementById("numero-cartao-modal");
  if (numCartao)
    numCartao.addEventListener(
      "input",
      (e) => (e.target.value = formatarCartao(e.target.value).substring(0, 19)),
    );

  const valCartao = document.getElementById("validade-cartao-modal");
  if (valCartao)
    valCartao.addEventListener(
      "input",
      (e) => (e.target.value = formatarValidade(e.target.value)),
    );
...
window.selecionarSubBadge = function (elemento) {
  ...
  container
    .querySelectorAll(".pay-sub-badge")
    .forEach((el) => el.classList.remove("active"));
  elemento.classList.add("active");
};
