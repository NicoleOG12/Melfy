document.addEventListener("DOMContentLoaded", () => {
  ...
  typeCards.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      if (leftPanel)
        leftPanel.style.backgroundImage = content[currentType].bgImage;
    });
  });
  ...
  if (form) {
    form.addEventListener("submit", async function (e) {
      ...
      function gerarCpfAleatorio() {
        ...
        const calcularDigito = (pesoInicial, arr) => {
          const soma = arr.reduce(
            (total, valor, i) => total + valor * (pesoInicial - i),
            0,
          );
          const resto = soma % 11;
      ...
      try {
        ...
        await alertSuccess(
          `Cadastro realizado com sucesso! Bem-vindo(a), ${nome.split(" ")[0] || nome}!`,
        );
  ...
  async function cadastrar(dados, tipo) {
    ...
    const res = await fetch(`http://localhost:38791/${endpoint}`, {
      method: "POST",
    ...
    if (!res.ok)
      throw new Error(retorno.message || retorno.erro || "Erro ao cadastrar");
    return retorno;
  }
  ...
  async function logar(email, senha, tipo) {
    ...
    const res = await fetch(`http://localhost:38791/${endpoint}/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
