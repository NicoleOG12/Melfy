...
export async function filtrarProdutos(produtos, inputElement, renderizarProdutosCallback, cardsWrapper, rolar = false) {
  ...
  if (!termo) {
    ...
    if (rolar) {
      const offset = 120;
      const topPos = cardsWrapper.getBoundingClientRect().top + window.scrollY - offset;
  ...
  try {
    const resposta = await fetch(`${API_URL}/produtos?search=${encodeURIComponent(termo)}`);
